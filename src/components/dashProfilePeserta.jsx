...
const ProfileCard = ({ title, children, className = "" }) => (
  <div className={`bg-[#F7F1E7] p-4 sm:p-6 rounded-xl ${className}`}>
    <h3 className="text-base sm:text-lg font-bold mb-3 sm:mb-4 text-[#5A5A3D]">{title}</h3>
    {children}
  </div>
);
      ...
      try {
        ...
        setProfileData({
          biodata: {
            ...
            tanggalLahir: formatDate(data.tanggal_lahir || data.tanggalLahir),
          },
          kelompok: {
            nama: data.kelompok || data.nama_kelompok || '',
            ketuaKelompok: data.ketua_kelompok || data.ketua || '',
          },
          pendamping: {
            nama: data.daplok || data.pendamping_nama || '',
            mentor: data.mentor || data.dpl_mentor || data.pendamping_mentor || '',
          },
        });
      } catch (err) {
