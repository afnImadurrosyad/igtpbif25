...
export default function NavbarDash({ 
  ...
  const isMobileMenuOpen = propIsMobileMenuOpen !== undefined ? propIsMobileMenuOpen : internalMobileMenu;
  ...
  return (
    <>
      ...
      {isMobile && (
        <div className="fixed top-0 left-0 right-0 h-16 bg-[#F7F1E7] border-b border-[#5a5a3d]/20 shadow-sm z-50 flex items-center justify-between px-4">
          <div className="flex items-center gap-2">
            <span className="text-lg font-bold text-[#5a5a3d] font-poppins">IGTTPB</span>
          </div>
          <button
      ...
      <div className={`
        ...
        ${isMobile 
          ? `border-l border-[#5a5a3d]/20 right-0 w-64 top-0 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}` 
          : `border-r border-[#5a5a3d]/20 left-0 top-0 ${isMinimized ? 'w-16' : 'w-64'}`
        }
      `}>
        ...
        {!isMobile && (
          <div className="h-16 flex items-center justify-between px-4 border-b border-[#5a5a3d]/20">
            {!isMinimized && (
              <div className="flex items-center gap-2">
                <span className="text-xl font-bold text-[#5a5a3d] font-poppins">IGTTPB</span>
              </div>
            )}
            <button
              onClick={handleToggle}
        ...
        {isMobile && (
          <div className="h-16 flex items-center justify-between px-4 border-b border-[#5a5a3d]/20">
            <div className="flex items-center gap-2">
              <span className="text-lg font-bold text-[#5a5a3d] font-poppins">Menu</span>
            </div>
            <button
              onClick={() => {
                if (typeof setIsMobileMenuOpen === 'function') {
          ...
          <div className="px-4 py-3 border-b border-[#5a5a3d]/20">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <div className="text-sm font-medium text-[#5a5a3d] font-poppins truncate">Jeremi</div>
                <div className="text-xs text-[#5a5a3d]/70 font-semibold font-poppins">Admin</div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Navigation */}
        <div className="py-4 px-2 overflow-y-auto scrollbar-hide" style={{ height: isMobile ? 'calc(100vh - 128px)' : 'calc(100vh - 128px)' }}>
          <nav className="space-y-1">
            {navItems.map((item, index) => {
              ...
              return (
                <div key={`nav-${index}-${item.href}`} className="relative group">
                  <button
                  ...
                  >
                    <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-white' : 'text-[#5a5a3d]'}`} />
