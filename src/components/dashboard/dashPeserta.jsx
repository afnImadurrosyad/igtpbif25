...
const DashPeserta = () => {
  ...
  if (loading) {
    return <div className="text-center p-10 font-semibold">Memuat daftar peserta...</div>;
  }
  if (error) {
    return <div className="text-center p-10 text-red-500 font-semibold">Error: {error}</div>;
  }
          ...
          <div className="space-y-2">
            {currentPeserta.length > 0 ? (
                ...
                return (
                    <div key={peserta.id}>
                      {/* Desktop View */}
                      <div className={`hidden md:flex items-center p-4 rounded-lg ${container}`}>
                        <p className={`w-1/12 ${textColor}`}>{(currentPage - 1) * articlesPerPage + index + 1}</p>
                        <p className={`w-3/12 font-medium ${textColor}`}>{peserta.nama}</p>
                        <p className={`w-2/12 ${textColor}`}>{peserta.nim}</p>
                        <p className={`w-2/12 ${textColor}`}>{peserta.kelompok}</p>
                        <p className={`w-2/12 ${textColor}`}>{peserta.daplok}</p>
                      </div>

                      {/* Mobile View - Card Style */}
                      <div className={`md:hidden p-4 rounded-lg ${container} space-y-2`}>
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <p className={`text-xs ${textColor} opacity-75 mb-1`}>
                            </p>
                            <p className={`font-semibold ${textColor} text-base`}>{peserta.nama}</p>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          <div>
                            <p className={`${textColor} opacity-75 text-xs`}>NIM</p>
                            <p className={`${textColor} font-medium`}>{peserta.nim}</p>
                          </div>
                          <div>
                            <p className={`${textColor} opacity-75 text-xs`}>Kelompok</p>
                            <p className={`${textColor} font-medium`}>{peserta.kelompok}</p>
                          </div>
                          <div className="col-span-2">
                            <p className={`${textColor} opacity-75 text-xs`}>Daplok</p>
                            <p className={`${textColor} font-medium`}>{peserta.daplok}</p>
                          </div>
                        </div>
                      </div>
                    </div>
